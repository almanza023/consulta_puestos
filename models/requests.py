...
class CombinedQuery(BaseModel):
    """Modelo para consulta combinada individual"""
    nuip: str = Field(..., description="Número único de identificación personal")
    fecha_expedicion: str = Field(..., description="Fecha de expedición en formato dd/mm/yyyy")
...
class MultipleCombinedRequest(BaseModel):
    """Modelo para múltiples consultas combinadas"""
    queries: List[CombinedQuery] = Field(..., description="Lista de consultas combinadas")
    delay: Optional[int] = Field(5, description="Delay en segundos entre consultas")
